...
This library provides a complete implementation of the [MCP server specification](https://modelcontextprotocol.io/specification/20.5.23-26/basic/transports) with Plug.
...
- `logging/setLevel` available if the MCP router is configured with `pubsub`. Logs can be sent to client with `Session.log_{level}(session, map_content)`. [See docs](https://modelcontextprotocol.io/specification/20.5.23-26/server/utilities/logging#log-levels).
